  ...
  checkoutButton.addEventListener("click", function () {
    ...
    for (const productId in cart.items) {
      if (cart.items.hasOwnProperty(productId)) {
        ...
        popupItemContainer.appendChild(productName);
        ...
        productSubtotal.innerHTML = formatCurrency(itemTotal);
        popupItemDetails.appendChild(productSubtotal);

        totalCheckout += cartItem.product.price * cartItem.quantity;
    ...
    const totalPembayaran = totalCheckout + totalCheckout * 0.11;
    strukTotalBayar.textContent =
      "Total Payment: " + formatCurrency(totalPembayaran);
