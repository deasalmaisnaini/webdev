...
class Cart {
    ...
    addToCart(product, quantity) {
        if (quantity > 0) {
            ...
            console.log("Quantity harus lebih besar dari 0 untuk menambahkan produk ke keranjang.");
        }
    }
    ...
    checkoutButton.addEventListener("click", function () {
        ...
        for (const productId in cart.items) {
            if (cart.items.hasOwnProperty(productId)) {
                ...
                const popupItemImage = document.createElement("img");
                popupItemImage.src = cartItem.product.image;
                popupItemImage.alt = cartItem.product.name;
                popupItemImage.className = "popup-item-image";
                popupItemContainer.appendChild(popupItemImage);
        ...
        strukPajak.textContent = `Pajak: ` + formatCurrency(totalCheckout * pajak);
        checkoutPopup.appendChild(strukPajak);


        const strukTotalBayar = document.createElement("p");
        strukTotalBayar.textContent = `Total Bayar : ` + formatCurrency(totalCheckout + (totalCheckout * pajak));
        ...
        checkoutPopup.appendChild(cancelButton);
    ...
    function updateCartItems() {
        ...
        for (const productId in cart.items) {
            if (cart.items.hasOwnProperty(productId)) {
                ...
                const productPrice = document.createElement("p");
                productPrice.textContent = `Price: ` + formatCurrency(cartItem.product.price);
